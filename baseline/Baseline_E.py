...
# 增强版：欧氏距离 K-means
def kmeans_E(X, k, max_iter=100, n_init=5):
    ...
    for _ in range(n_init):
        centroids = kmeans_plusplus_e(X_scaled, k)
        for _ in range(max_iter):
            ...
            for idx in range(n_samples):
                sample = X_scaled[idx]
                distances = [e_distance(sample, centroid) for centroid in centroids]
                cluster_idx = np.argmin(distances) # 欧氏距离取最小
    ...
    for run in range(1, num_runs + 1):
        for name, loader in datasets.items():
            try:
                ...
                clusters, centroids, y_pred = kmeans_E(X, k_classes)
